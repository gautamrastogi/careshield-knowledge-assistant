...
CareShield is a small governed GenAI learning project built on synthetic
healthcare documents. It demonstrates the platform controls around RAG:
document parsing, chunking, embeddings, Chroma vector retrieval, role-based
policy filtering before model input, PII redaction, a model gateway
abstraction, Pydantic structured responses, citation/grounding evals, and
trace events.
...
embeddings, indexes those chunks in Chroma, builds user
...
The vector store also uses an adapter, so Chroma is the default learning
backend while the architecture can still map to OpenSearch or pgvector later.
...
- Chroma is the default vector DB, with an adapter boundary that maps to
  pgvector or OpenSearch Serverless later.
- Evals and traces explain whether the answer is cited, grounded, redacted, and
  policy-safe.
- Golden eval cases run in CI, so regressions are caught as the project grows.
...
spans, add Terraform, expand the golden eval dataset, and add a persisted
Chroma/OpenSearch deployment profile.
