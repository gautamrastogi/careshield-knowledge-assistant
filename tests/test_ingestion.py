...
def test_chunking_and_document_building_preserve_policy_metadata() -> None:
    ...
    assert documents[0].sensitivity == contracts.schema.Sensitivity.clinical
    assert contracts.schema.Role.nurse in documents[0].allowed_roles
    assert contracts.schema.Role.external_vendor not in documents[0].allowed_roles
